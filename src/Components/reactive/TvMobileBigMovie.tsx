...
import { IGetMovieDetailResult, TvDetails } from "../../api";
import { match, useHistory } from "react-router-dom";
import { theme } from "../../theme";
import { getTime, getYears, makeImagePath } from "../../utilitis";
import { IMovieId } from "../TvSlider";
...
interface IMobileBigMovieProp {
  tvDetail: TvDetails | undefined;
  bigMovieMatch: match<IMovieId>;
  genres: any;
  title: string;
}

export default function TvMobileBigMovie({
  tvDetail,
  bigMovieMatch,
  genres,
  title,
  ...
  return (
    <>
      <BigMovie isPc={isPc} layoutId={bigMovieMatch.params.tvId + title}>
        {!tvDetail ? (
            ...
            <BigMovieBottom>
              <InFoWrapper>
                <BigTitle>
                  ...
                  <OriginalTitle>{tvDetail?.original_name}</OriginalTitle>
                </BigTitle>
                <SmallInfo>
                  {" "}
                  <div>{getYears(tvDetail?.first_air_date || "")} </div>
                </SmallInfo>
                <Runtime>
                  {" "}
                  {`에피소드 ${
                    tvDetail?.last_episode_to_air.episode_number as number
                  }개`}
                </Runtime>
